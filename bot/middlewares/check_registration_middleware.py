...
logger = structlog.get_logger(name="check_registration_middleware")
    ...
    async def __call__(self, handler: Message | CallbackQuery, event, data):
        ...
        if user_id:
            try:
                ...
                logger.error("error while checking user data", extra_data={"user_id": user_id}, exc_info=e)
                return

        if created_new_user:
            logger.info("created new user", extra_data={"user_id": user_id})
