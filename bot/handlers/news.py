...
logger = logging.getLogger("handlers.news")
...
async def process_news_content(message: Message, state: FSMContext):
    ...
    logger.info("someone trying to send news", extra=logging_extra)

    if message.media_group_id is not None:
        logger.info("someone tried to send media_group, declined", extra=logging_extra)
    ...
    logger.info("begging the mailing", extra=logging_extra)
    await news_service.send(message, await users_service.get_objects_field("telegram_id"))
    logger.info("mailing started", extra=logging_extra)
